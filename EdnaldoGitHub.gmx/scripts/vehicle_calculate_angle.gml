/// vehicle_calculate_angle(x, y, angle)
 ...
 // Push down sensors in order to know the angle:
    repeat(25)
    {
           if(vehicle_collision(floor(xPoint[0]), floor(yPoint[0]), collision_layer) == false)
           {
              xPoint[0] += dsin(argument2);
              yPoint[0] += dcos(argument2);
           }
           if(vehicle_collision(floor(xPoint[1]), floor(yPoint[1]), collision_layer) == false)
           {
